...
const Section5 = () => {
  ...
  return (
    <div className="flex flex-col items-center text-center pt-15"> 
      ...
      <motion.div initial={{y:20,opacity:0}} animate={{y:0,opacity:1}} transition={{duration:1,delay:0.5,ease:"easeOut"}} className='flex flex-col max-w-md md:max-w-125 items-center text-center '>
      ...
      <Marquee gradient={true} pauseOnHover={false} speed={60} className="py-15 flex items-center">
        {assets.car_logos.map((img, index) => (
          <img key={index} src={img.logo} alt={img.name} onClick={()=>handleClick(img.name)} className="w-[150px] md:w-[200px] h-auto object-contain mx-25 cursor-pointer"/>
        ))}
      </Marquee>
    </div>
  )
